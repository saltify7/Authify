...
const getFilterSettings = (sdk: SDK): Result<typeof filterSettings> => {
  return { kind: "Ok", value: { ...filterSettings } };
};

// Function to set filter settings
const setFilterSettings = (sdk: SDK, settings: typeof filterSettings): Result<void> => {
...
const shouldFilterRequest = (req: any): boolean => {
...
const isPluginGeneratedRequest = (sdk: SDK, req: any): boolean => {
...
const saveAuthHeaders = (sdk: SDK, headers: string): Result<void> => {
...
const getAuthHeaders = (sdk: SDK): Result<string> => {
...
const setSelectedScope = (sdk: SDK, scope: string): Result<void> => {
  selectedScope = scope;
...
const sendHeadersToAuthify = async (sdk: SDK, requestId: string): Promise<Result<{ headers: string; count: number }>> => {
  try {
    // Get the request directly using the request ID
    const requestResponse = await sdk.requests.get(requestId.toString());
    if (!requestResponse) {
...
const getResponseContentLength = (resp: any, respRaw: string): number => {
...
const applyHeadersToReplay = async (sdk: SDK, requestId: string): Promise<Result<{ id: string }>> => {
  try {
    // Check if we have auth headers configured
    if (!storedAuthHeaders.trim()) {
      return { kind: "Error", error: "No authentication headers configured. Please add headers in the Config tab first." };
...
const extractResponseBody = (responseRaw: string): string => {
...
const getLocationHeader = (responseRaw: string): string | undefined => {
...
const compareResponses = (originalCode: number, originalLength: number, modifiedCode: number, modifiedLength: number, originalRespRaw: string, modifiedRespRaw: string): "same" | "different" | "similar" | "unknown" => {
...
const processNewRequest = async (sdk: SDK, req: any, resp: any): Promise<Row | null> => {
  ...
  const isPluginRequest = isPluginGeneratedRequest(sdk, req);
  if (isPluginRequest) {
  ...
  if (selectedScope && selectedScope.trim() !== "") {
    try {
      const isInScope = sdk.requests.inScope(req);
      if (!isInScope) {
        sdk.console.log(`Skipping request - URL ${url} not in scope ${selectedScope}`);
        return null;
      }
    } catch (scopeError) {
  ...
  if (isPluginEnabled && storedAuthHeaders.trim() && !modifiedRequestIds.has(id)) {
